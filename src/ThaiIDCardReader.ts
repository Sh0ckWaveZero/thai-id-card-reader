        ...
        if (changes) {
          if (
            ...
            const connectionResult =
              await CardReaderConnection.attemptConnection(reader);

            if (!connectionResult.connected) {
              that.eventEmitter.emit(
                "READ_ERROR",
                RESPONSE_MESSAGES.CONNECTION_FAILED
              );
              return;
            }
            ...
            if (!protocol) {
              that.eventEmitter.emit(
                "READ_ERROR",
                "No protocol available for card connection"
              );
              return;
            }
            ...
            const sendRawCommand = async (
              ...
              return CommandSender.sendRawCommand(
                reader,
                protocol,
                data,
                that.readTimeout,
            ...
            try {
              ...
              const isCardReady = await PCSCErrorHandler.validateCardState(
                reader
              );
              if (!isCardReady) {
              ...
              data.dateOfBirth = moment(
                await getData(apdu.dateOfBirth),
                "YYYYMMDD"
              )
                .add(-543, "years")
                .format("YYYY-MM-DD");
              data.issueDate = moment(await getData(apdu.issueDate), "YYYYMMDD")
                .add(-543, "years")
                .format("YYYY-MM-DD");
              data.expireDate = moment(
                await getData(apdu.expireDate),
                "YYYYMMDD"
              )
                .add(-543, "years")
                .format("YYYY-MM-DD");
              ...
              for (let i = 0; i < apdu.photos.length; i++) {
                try {
                  ...
                  logger.debug(
                    `Photo segment ${i + 1}/${apdu.photos.length} read`
                  );
                } catch (error) {
              ...
              if (PCSCErrorHandler.isProtocolMismatchError(cardError)) {
                logger.error(
                  "Card reading error (protocol mismatch):",
                  cardError.message
                );
                ...
                const errorInfo = PCSCErrorHandler.analyzeError(cardError);
                logger.error(
                  "Card reading error (citizen ID):",
                  errorInfo.message
                );
                that.eventEmitter.emit("READ_ERROR", errorInfo.userMessage);
              } else {
                logger.error(
                  "Card reading error (general):",
                  cardError.message
                );
                that.eventEmitter.emit(
                  "READ_ERROR",
                  `Card reading failed: ${cardError.message}`
                );
              }
